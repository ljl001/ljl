...
<div class="margin add_administrator" id="page_style">
    ...
    <ul>
     ...
     <input type="text" class="input-text col-xs-12" value="" placeholder="" id="user-name" name="u_user" datatype="*2-16" nullmsg="用户名不能为空"></div>
    <div class="col-4"> <span class="Validform_checktip"></span></div>
     </li>
     <li class="clearfix">
     <label class="label_name col-xs-2 col-lg-2"><i class="c-red">*</i>初始密码：</label>
	 <div class="formControls col-xs-6">
	 <input type="password" placeholder="密码" name="pwd" autocomplete="off" value="" class="input-text col-xs-12" datatype="*6-20" nullmsg="密码不能为空">
	</div>
     <div class="col-4"> <span class="Validform_checktip"></span></div>
     </li>
     <li class="clearfix">
	...
	<input type="password" placeholder="确认新密码" autocomplete="off" class="input-text Validform_error  col-xs-12" errormsg="您两次输入的密码不一致！" datatype="*" nullmsg="请再输入一次新密码！" recheck="userpassword" id="newpassword2" name="newpassword2">
     ...
     <li class="clearfix">
      ...
      <div class="formControls col-xs-6"> <span class="select-box" style="width:150px;">
				<select class="select" name="r_name" size="1">
     ...
     <li class="clearfix">
			...
			<div class="formControls col-xs-6">
				<textarea name="count" cols="" rows="" class="textarea col-xs-12" placeholder="说点什么...100个字符以内" dragonfly="true" onkeyup="checkLength(this);"></textarea>
...
<script type="text/javascript">
	$("#Add_submit").click(function(){
		var user = $('input[name="u_user"]').val();
		var pwd  = $('input[name="pwd"]').val();
		var sex = $('input:radio[name="sex"]:checked').val();
		var phone = $('input[name="phone"]').val();
		var email = $('input[name="email"]').val();
		var r_name = $('select option:selected').val();
		var count = $('.textarea').val();
		// alert(email);
		$.ajax({
			url:'../index.php?r=admin/brands',
			data:{user:user,pwd:pwd,sex:sex,phone:phone,email:email,r_name:r_name,count:count},
			type:'post',
			success:function(msg){
				...
				location.href = '../index.php?r=admin/admin';
			}
		})
